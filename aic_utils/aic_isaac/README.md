...
List available environments (`Intrinsic-AIC-Task-v0` RL Environment is provided as reference):
```bash
isaaclab -p aic/aic_utils/aic_isaac/intrinsic_aic_isaaclab/scripts/list_envs.py
```
...
```bash
isaaclab -p aic/aic_utils/aic_isaac/intrinsic_aic_isaaclab/scripts/rsl_rl/train.py \
    --task Intrinsic-AIC-Task-v0 --num_envs 1 --enable_cameras
```
